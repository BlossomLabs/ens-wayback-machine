  ...
  return getFromENSGraph(
    ...
    async (result: any) => {
      if (result.data.registrationEvents.length > 0) {
        ...
        const filtereDomainRenewals = result.data.registrationEvents.filter(
          (obj: object) => Object.keys(obj).length > 0,
        );

        // Obtain block timestamp and add event type
        const processedExpiryExtendeds = await Promise.all(
          filtereDomainRenewals.map(async (obj: any) => {
            const block = await ethereumProvider.getBlock(obj.blockNumber);
            const from = (await ethereumProvider.getTransaction(
              obj.transactionID,
            ))!.from;
            ...
            return {
              ...
              eventType: "domainRenewal",
              from: from,
              fromLookedUp: fromLookedUp,
            };
          }),
        );
